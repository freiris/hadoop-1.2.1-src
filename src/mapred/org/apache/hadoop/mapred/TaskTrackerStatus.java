...
// yongxiang: begin
  /*
   * this concurrentHashMap records the network data transfer in the copy phase 
   */
  static ConcurrentHashMap<String, Long> networkDataTransMap =
      new ConcurrentHashMap<String, Long>();

  String networkDataTransString; // used for serialization
  //yongxiang: end
  ...
  public void write(DataOutput out) throws IOException {
 // yongxiang: begin
    setNetworkDataTransString();
    Text.writeString(out, networkDataTransString);
    // yongxiang: end
  ...
  public void readFields(DataInput in) throws IOException {
    // yongxiang: begin
    this.networkDataTransString = Text.readString(in);
    // yongxiang: end
...
// yongxiang: begin
  public void setNetworkDataTransString() {
    String key;
    Long value;
    String entry;

    networkDataTransString = "";
   // synchronized (TaskTrackerStatus.networkDataTransMap) {
    for (Iterator iter =
        TaskTrackerStatus.networkDataTransMap.keySet().iterator(); iter
        .hasNext();) {
      key = (String) iter.next();
      value = TaskTrackerStatus.networkDataTransMap.get(key);
      entry = key + "," + value + ";";
      networkDataTransString = networkDataTransString + entry;
    }
    // we should immediately clear the NetworkDataTransTable
    TaskTrackerStatus.networkDataTransMap.clear();
    // }
    System.out.println("yongxiang: TransString = " + networkDataTransString);
  }
  // yongxiang: end
}
